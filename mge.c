...
int AddWordToVocab(char *word, int is_non_comp) {
  ...
  if(is_non_comp) return vocab_size - 1;   //for non_compositional entity, neither characters nor radicals are used.
...
void *TrainModelThread(void *id) {
  ...
  while (1) {   
    ...
    if (cw) {
      // Use the target words' components
      ...
      for (c = 0; c < layer1_size; c++) {
        neu1[c] /= cw;
        if (comp_list_cnt > 0)
          neu1[c] = (neu1[c] + neucomp[c] / comp_list_cnt) / 2;
      }
      // negative sampling
  ...
  for (a = 0; a < vocab_size; a++) {
    if (vocab[a].word != NULL)
      fprintf(fo, "%s ", vocab[a].word);
  		...
  		continue;
  	for (b = 0; b < layer1_size; b++) vec[b] = syn0[b + a * layer1_size];
  	if (vocab[a].character_size){
  		for (b = 0; b < vocab[a].character_size; b++) {
  			char_id = vocab[a].character[b];
  			for (c = 0; c < layer1_size; c++) vec[c] += synchar[c + char_id * layer1_size] / vocab[a].character_size;
  		}
  	}
    if (binary)
      for (b = 0; b < layer1_size; b++) fwrite(&vec[b], sizeof(real), 1, fo);
    else
      for (b = 0; b < layer1_size; b++) fprintf(fo, "%lf ", vec[b]);
    fprintf(fo, "\n");
  }
  fclose(fo);
  free(table);
